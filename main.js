...
(async () => {
  ...
  const csvHeader='Name,Rating,Reviews,Category,Hours,Address,Website,Phone,Url\n';
  const csvRows=results.map(r=>`${r.name},${r.rating},${r.reviews},${r.category},${r.hours},${r.address},${r.website},${r.phone},${r.url}`).join('\n');
